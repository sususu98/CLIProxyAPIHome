...
func AppLogRecordFromPayload(clientIP string, payload string) (*AppLogRecord, error) {
...
func (r *Repository) AppendAppLog(ctx context.Context, clientIP string, payload string) (*AppLogRecord, error) {
	db, errDB := r.database()
	if errDB != nil {
		return nil, errDB
	}

	record, errRecord := AppLogRecordFromPayload(clientIP, payload)
	if errRecord != nil {
