...
| `related.request_log` | object | request log 关联信息；可用时包含 `home_ip` 和 `home_port`。只有当前 Home 能找到本地日志文件时 `available=true` 并返回 download URL。 |
...
`items[]` 包含 `id`、`request_id`、`timestamp`、`home_ip`、`home_port`、`file_name`、`size_bytes`、`available`、`provider`、`model`、`status` 和 `download_url`。只有当前节点能找到本地文件时，`available=true` 且 `download_url` 非空；文件不存在或属于远端节点时返回 `available=false`。实际下载仍使用已有 `GET /request-log-by-id/:id`；当 `home_port` 可用时，生成的下载 URL 会同时带上 `home_ip` 和 `home_port`。
