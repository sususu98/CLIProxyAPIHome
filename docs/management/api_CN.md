...
额度快照接口是纯只读 DB 视图。读取不会请求上游 Provider、刷新 OAuth token、改变调度优先级或消费队列。凭证存在但从未产生快照且从未尝试采集时返回 `quota_status=unknown`、`freshness=never`、空窗口和 HTTP `200`。如果第一次采集在获得任何可用额度事实前失败，则返回 `quota_status=error`、`freshness=never` 和 `collection_status=failed`。Provider 明确不在当前采集规划内时返回 `unsupported`。凭证删除后不再可见，对应额度表记录同时删除。
...
当前被动采集从 CPA usage 事件的 `response_headers` 中提取受限 `quota_headers`。Home 只保留 Codex `X-Codex-*` 额度 Header allowlist 和有长度上限的 upstream request ID，并在 usage payload 入库前删除 raw `response_headers`。Codex Header 观测与 usage record 在同一事务中归一化并 upsert，迟到事件不能覆盖更新快照。更新的 Header 观测会使正在运行的主动探测租约失效；部分 Header 更新只保留仍有效的旧窗口，已过期窗口不会参与新快照状态汇总。
